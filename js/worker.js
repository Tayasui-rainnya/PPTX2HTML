...
function genShape(node, slideLayoutSpNode, slideMasterSpNode, id, name, idx, type, order, warpObj) {
    ...
    if (shapType !== undefined) {
        ...
        result += renderOfficeMathOutsideTextBody(node);
        result += "</div>";
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        switch (child.name) {
            ...
            case "m:oMathPara":
                ...
                break;
            default:
                content += renderOfficeMathDescendants(child.node);
        }
    }
...
/**
 * Renders formulas stored in extension or alternate-content nodes that are direct
 * shape children, while excluding the text body already rendered as paragraphs.
 */
function renderOfficeMathOutsideTextBody(shapeNode) {
    var content = "";
    var children = getOrderedChildNodes(shapeNode);
    for (var i = 0; i < children.length; i++) {
        if (children[i].name !== "p:txBody") {
            content += isOfficeMathNode(children[i].name) ?
                genMathElement(children[i].node, children[i].name === "m:oMathPara") :
                renderOfficeMathDescendants(children[i].node);
        }
    }
    return content;
}
...
/**
 * Finds Office Math below an OOXML wrapper such as AlternateContent or an
 * extension node, stopping at each formula to prevent duplicate rendering.
 */
function renderOfficeMathDescendants(node) {
    var content = "";
    var children = getOrderedChildNodes(node);
    for (var i = 0; i < children.length; i++) {
        if (isOfficeMathNode(children[i].name)) {
            content += genMathElement(children[i].node, children[i].name === "m:oMathPara");
        } else {
            content += renderOfficeMathDescendants(children[i].node);
        }
    }
    return content;
}
...
/**
 * Identifies Office Math elements independently of their namespace prefix so
 * extension markup that aliases the math namespace is handled consistently.
 */
function isOfficeMathNode(name) {
    return name === "m:oMath" || name === "m:oMathPara" || /:oMath(Para)?$/.test(name);
}
